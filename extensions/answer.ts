  ...
  "questions": [
    {
      ...
      "context": "Optional multi-line context including options, tradeoffs, and recommendations. Use \\n for line breaks."
    }
  ]
}
...
- Be concise with the question text itself
- Include context when there are options, tradeoffs, constraints, recommendations, or background that helps the user answer
- Preserve the FULL structure of options and recommendations in the context field using \\n for line breaks
- Keep bullet points, labeled options (A, B, C), and recommendations verbatim
- If a question has a source-of-truth header or section label, include it in the context
- If no questions are found, return {"questions": []}

Example input:
Q1. Is Birdeye a hard dependency, or just preferred?
If Birdeye cannot provide a metric cleanly, do we:
- A) return null
- B) approximate
- C) fall back to Solana RPC / another source
Recommended: C for balances, A for bot usage unless you define it, B only when explicitly marked approximate.
Reason: some of your metrics are not first-class Birdeye outputs.

Example output for that input:
{
  "questions": [
    {
      "question": "Is Birdeye a hard dependency, or just preferred?",
      "context": "If Birdeye cannot provide a metric cleanly, do we:\n- A) return null\n- B) approximate\n- C) fall back to Solana RPC / another source\n\nRecommended: C for balances, A for bot usage unless you define it, B only when explicitly marked approximate.\nReason: some of your metrics are not first-class Birdeye outputs."
    }
  ]
}`;
...
class QnAComponent implements Component {
	...
	private submit(): void {
		...
		for (let i = 0; i < this.questions.length; i++) {
			...
			if (q.context) {
				for (const ctxLine of q.context.split("\n")) {
					parts.push(`> ${ctxLine}`);
				}
			}
			parts.push(`A: ${a}`);
	...
	render(width: number): string[] {
		...
		// Context if present (supports multi-line with \n)
		if (q.context) {
			lines.push(padToWidth(emptyBoxLine()));
			for (const ctxLine of q.context.split("\n")) {
				const contextText = this.gray(`> ${ctxLine}`);
